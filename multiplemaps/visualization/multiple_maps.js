...
d3.json(filename, function(maps) {
	...
	var trans_len = 500
    var zoom_level = []
	var nodes = [], links = [], link = [], node1 = [], node2 = [], vis = [], force = []
    ...
    function redraw() {
        ...
        if(d3.event && !d3.event.type) {
            ...
            force[zoom_map].stop()
            force[zoom_map]
            .charge(-30 / (d3.event.scale * zoom_level[zoom_map]))
            .gravity(.1 / (d3.event.scale * zoom_level[zoom_map]))
            .linkDistance(function(d) { return d.type == 1 ? 5 / (d3.event.scale * zoom_level[zoom_map]) : 70 / (d3.event.scale * zoom_level[zoom_map])})
            force[zoom_map].start()
        }
        ...
        for(map_ind = 0; map_ind < cur_map.length; map_ind++) {
            ...
            link[map_ind]
            .attr("x1", function(d) { return cur_fun_xx(d.source.x) })
            .attr("x2", function(d) { return cur_fun_xx(d.target.x) })
            .attr("y1", function(d) { return cur_fun_yy(d.source.y) })
            .attr("y2", function(d) { return cur_fun_yy(d.target.y) })
        }
    }
	...
	function zoom_to_word(word) {
		...
		var map_ind, i
		for(map_ind = 0; map_ind < cur_map.length; map_ind++) {
			for(i = 0; i < nodes[map_ind].length; i++) {
				if(!nodes[map_ind][i].fixed) {
					if(nodes[map_ind][i].word.toUpperCase().localeCompare(word.toUpperCase()) == 0) {

                        // Translate node and word to the center
                        zoom_level[map_ind] = cur_map.length <= 3 ? 3 : 5
                        var trans_x = (vis_width  / 2) - zoom_level[map_ind] * nodes[map_ind][i].x
						var trans_y = (vis_height / 2) - zoom_level[map_ind] * nodes[map_ind][i].y
                        fun_xx[map_ind] = d3.scale.linear().domain([0, 1]).range([0 + trans_x, zoom_level[map_ind] + trans_x])
                        fun_yy[map_ind] = d3.scale.linear().domain([0, 1]).range([0 + trans_y, zoom_level[map_ind] + trans_y])
					}
				}
			}
        
            // Restart force-directed layouts
            force[map_ind].stop()
            force[map_ind]
            .charge(-30 / zoom_level[map_ind])
            .gravity(.1 / zoom_level[map_ind])
